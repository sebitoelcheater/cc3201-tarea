...
create view cc320114_db.dlikes as
	Select tag.tag as Actor,SUM(CASE WHEN tag_liking.type=0 THEN 1 ELSE 0 END)as Dislikes
	From cc320125_db.tag,cc320125_db.tag_liking,cc320114_db.actores
	Where tag.id=tag_liking.id_tag and tag.tag=actores.nombre
	Group by(Actor);

create view cc320114_db.likes as
	Select tag.tag as Actor,SUM(CASE WHEN tag_liking.type=1 THEN 1 ELSE 0 END)as Likes
	From cc320125_db.tag,cc320125_db.tag_liking,cc320114_db.actores
	Where tag.id=tag_liking.id_tag and tag.tag=actores.nombre
	Group by(Actor);

create view cc320114_db.Tag_de_actores as
	Select likes.Actor as Actor, likes.Likes as Likes, dlikes.Dislikes as Dislikes
	From cc320114_db.likes, cc320114_db.dlikes
	Where likes.Actor=dlikes.Actor
	Order by(Actor)asc;
